...
class SpeakerForm extends React.Component {
  ...
  handleCurrentStatus = (event) =>
    (this.speaker.currentStatus.time = event.target.value);
  ...
  checkBoxDisable = (disable, iStart, iEnd) => {
    if (disable) {
      for (let i = iStart; i < iEnd; i++) {
        let inputCheked = document
          .getElementsByClassName("MuiFormControlLabel-root")
          [i].getElementsByTagName("input")[0].checked;
        if (!inputCheked) {
          document
            .getElementsByClassName("MuiFormControlLabel-root")
            [i].classList.add("Mui-disabled");
          document
            .getElementsByClassName("MuiFormControlLabel-root")
          ...
          document
            .getElementsByClassName("MuiFormControlLabel-root")
            [i].getElementsByTagName("input")[0].disabled = "true";
        }
      }
    } else {
      for (let i = iStart; i < iEnd; i++) {
        ...
        if (!inputCheked) {
          document
            .getElementsByClassName("MuiFormControlLabel-root")
            [i].classList.remove("Mui-disabled");
          document
            .getElementsByClassName("MuiFormControlLabel-root")
            [i].getElementsByTagName("span")[0]
          ...
          document
            .getElementsByClassName("MuiFormControlLabel-root")
            [i].getElementsByTagName("input")[0].disabled = false;
        }
      }
    }
  };
  ...
  handleParticipantsFormat = (event) =>
    (this.speaker.participants_format = event.target.value);
  ...
  handleExperienceIndusrtry = (event) => {
    ...
    if (checked) {
      ...
      else if (experienceIndusrtry.exp3 === "") {
        experienceIndusrtry.exp3 = val;
      }
    } else {
      if (experienceIndusrtry.exp1 === val) experienceIndusrtry.exp1 = "";
      else if (experienceIndusrtry.exp2 === val) experienceIndusrtry.exp2 = "";
      else if (experienceIndusrtry.exp3 === val) {
        experienceIndusrtry.exp3 = "";
      }
    }
    let disable;
    if (
      experienceIndusrtry.exp1 !== "" &&
      experienceIndusrtry.exp2 !== "" &&
      experienceIndusrtry.exp3 !== ""
    )
      disable = true;
    else disable = false;
  ...
  handleBioText = (event) => {
    ...
    if (text.match(/\S+/g)) {
      if (
        text.match(/\S+/g).length !== null &&
        text.match(/\S+/g).length > 101
      ) {
  ...
  handleAttach = (event) => {
    ...
    if (type !== "jpeg" && type !== "png") {
      this.setState({
        imgError: "Accepted file format is JPG or PNG *",
        imgErrorhiden: "yes",
      });
      this.refs.imgText.style.borderBottom = "2px solid #f20707";
    } else if (fileInput.files[0].size > 2097152) {
      this.setState({
        imgError: "Attachment failure. The maximum file size is 2 MB. *",
        imgErrorhiden: "yes",
      });
      this.refs.imgText.style.borderBottom = "2px solid #f20707";
    } else {
  ...
  handleSubmit = () => {
    ...
    let error = "";
    if (
      opportunities.workshop === false &&
      opportunities.mentorships === false &&
      opportunities.camfireTalk === false &&
      opportunities.keynotSpeach === false
    )
      error = "opportunities";
    ...
    else if (email === "" || EmailValidator.validate(email) !== true)
      error = "email";
    else if (mobile === "") error = "mobile";
    else if (currentStatus.time === "" || currentStatus.doing === "")
      error = "currentStatus";
    else if (
      experienceFild.exp1 === "" &&
      experienceFild.exp2 === "" &&
      ...
      error = "experienceFild";
    else if (
      experienceIndusrtry.exp1 === "" &&
      experienceIndusrtry.exp2 === "" &&
      experienceIndusrtry.exp3 === ""
    )
      error = "experienceIndusrtry";
    ...
    if (error !== "") {
      ...
      fetch(
        "https://www.apis.seasidestartupsummit.com/aplications/speakerimg.php",
        {
          method: "POST",
          body: formData,
        }
        ...
        .then((data) => {
          ...
          if (data.resp !== "error") {
            ...
            fetch(
              "https://www.apis.seasidestartupsummit.com/aplications/speaker.php",
              {
                method: "POST",
              ...
              .then((data) => {
                if (data.resp === "ok") {
                  ...
                  window.localStorage.setItem(
                    "msg",
                    "Your application is taken forward. SSS Team will contact you if you are shortlisted."
                  );
    ...
    return (
      <Fragment>
        <div className="speakerInfo">
          <h3>Apply Here</h3>
          <p>
            Please complete the speaker request form and tell us about you and
            your seminars as much as possible. Please also upload a professional
            headshot image, as we may use this if you have been chosen. Please
            note that we cannot accept all speaker requests and applications.
          </p>
        </div>
        ...
        <div className="form_body">
          <div className="form_body_grid">
            <h4 ref="opportunities">
              Which of the following opportunities are you interested in? *
            </h4>
            <FormGroup>
              <FormControlLabel
                control={
                  <Checkbox
                    ...
                    value="workshop"
                    color="primary"
                    onChange={(e) => this.handleOpportunities(e, 1)}
                  />
                }
                label="Workshop"
              />
              <FormControlLabel
                control={
                  <Checkbox
                    ...
                    value="mentorship"
                    color="primary"
                    onChange={(e) => this.handleOpportunities(e, 2)}
                  />
                }
                label="Mentorship"
              />
              <FormControlLabel
                control={
                  <Checkbox
                    ...
                    value="campfireTalk"
                    color="primary"
                    onChange={(e) => this.handleOpportunities(e, 3)}
                  />
                }
                label="Campfire Talk"
              />
              <FormControlLabel
                control={
                  <Checkbox
                    ...
                    value="keynote"
                    color="primary"
                    onChange={(e) => this.handleOpportunities(e, 4)}
                  />
                }
          ...
          <div className="form_body_grid" style={{ marginTop: "40px" }}>
            <h4 ref="participants_format">
              Which participation format would you prefer? *
            </h4>
            <RadioGroup
              aria-label="participants_format"
              name="participants_format"
              onChange={this.handleParticipantsFormat}
            >
              <FormControlLabel
                value="online"
          ...
          <div className="form_body_flex">
            <div>
              <TextField
                ref="firstName"
                onChange={(e) => this.handleBasicButtons(e, 1)}
                label="First Name *"
              />
              <TextField
                ref="lastName"
                onChange={(e) => this.handleBasicButtons(e, 6)}
                label="Family Name *"
              />
              <TextField
                ref="nationality"
                onChange={(e) => this.handleBasicButtons(e, 2)}
                label="Nationality *"
              />
              <TextField
                ref="mobile"
                onChange={(e) => this.handleBasicButtons(e, 3)}
                label="Phone Number *"
              />
              <TextField
                ref="email"
                onChange={(e) => this.handleBasicButtons(e, 7)}
            ...
            <div>
              <div className="gender">
                ...
                >
                  <span className="genderSpan" ref="gender">
                    Gender *
                  </span>
                  <FormControlLabel
                    value="male"
              ...
              <TextField
                onChange={(e) => this.handleBasicButtons(e, 4)}
                label="Facebook Profile link (If available)"
              />
              <TextField
                onChange={(e) => this.handleBasicButtons(e, 5)}
                label="LinkedIn Profile link (If available)"
              />
            </div>
          </div>
        </div>
        ...
        <div className="form_body">
          <div className="form_body_grid2">
            ...
            <RadioGroup
              ...
              name="currentStatus"
              onChange={this.handleCurrentStatus}
            >
              <FormControlLabel
            ...
            <div className="textButtonBox">
              <TextField
                onChange={(e) => this.handleCurrentStatusText(e, 1)}
                label="What is it that you do... *"
              />
              <TextField
                onChange={(e) => this.handleCurrentStatusText(e, 2)}
                label="Company name"
              />
              <TextField
                onChange={(e) => this.handleCurrentStatusText(e, 3)}
                label="Position"
              />
            </div>
          </div>
          <div className="form_body_flex2">
            <div>
              <h4 ref="experienceFild">
                Field of Experience * <span>(Select Max. 3)</span>{" "}
              </h4>
              <FormGroup>
                <FormControlLabel
                  control={
                ...
                <FormControlLabel
                  control={
                    <Checkbox
                      onChange={this.handleExperience}
                      value="Business and SDG alignment"
                      color="primary"
                    />
                  }
                  label="Business and SDG alignment"
                />
                <FormControlLabel
                  control={
                    <Checkbox
                ...
                <FormControlLabel
                  control={
                    <Checkbox
                      onChange={this.handleExperience}
                      value="Fundraising: Crowdfunding"
                      color="primary"
                    />
                  }
                ...
                <FormControlLabel
                  control={
                    <Checkbox
                      onChange={this.handleExperience}
                      value="Fundraising: Venture Capital"
                      color="primary"
                    />
                  }
                ...
                <FormControlLabel
                  control={
                    <Checkbox
                      onChange={this.handleExperience}
                      value="Presentations and pitching"
                      color="primary"
                    />
                  }
                  label="Presentations and pitching"
                />
                <FormControlLabel
                  control={
                    <Checkbox
            ...
            <div>
              <h4 ref="experienceIndusrtry">
                Industry or Technology of Experience *{" "}
              ...
              <FormGroup>
                <FormControlLabel
                  control={
                    <Checkbox
                      onChange={this.handleExperienceIndusrtry}
                      value="AgroTech"
                      color="primary"
                    />
                  }
                ...
                <FormControlLabel
                  control={
                    <Checkbox
                      onChange={this.handleExperienceIndusrtry}
                      value="Greentech-Cleantech"
                      color="primary"
                    />
                  }
                  label="Greentech & Cleantech"
                />
                <FormControlLabel
                  control={
                    <Checkbox
          ...
          <div>
            <div className="textAreaBox">
              <div>
                <h6 ref="personalBio">
                  <i>
                    <img src={require("./img/minIcon/person.png")} alt="icon" />
                  </i>{" "}
                  Personal Bio *{" "}
                  <span ref="textArea1">
                    {" "}
                    (Max {this.state.bioTextCout} words)
                  </span>
                </h6>
              </div>
              <textarea
                ref="bio"
                placeholder="Please keep in mind, that the provided information will be used in all further communications and marketing materials"
                maxLength={this.state.bioLenght}
                onChange={this.handleBioText}
              ></textarea>
            </div>
          </div>
          <div className="attachFile" ref="img">
            ...
            <label htmlFor="attachPhoto" className="attachText" ref="imgText">
              {this.state.imgError}
            </label>
            <input
              type="file"
        ...
        <div className="form_body">
          <div className="form_body_grid2">
            <h4 ref="topEvent">
              Top 3 Events Where You Have Been Participated as a Speaker/Mentor
              *
            </h4>
            <div className="textButtonBox">
              <TextField
                onChange={(e) => this.handleOtherEvent(e, 1)}
                label="Name of the event"
              />
              <TextField
                onChange={(e) => this.handleOtherEvent(e, 2)}
                label="Name of the event"
              />
              <TextField
                onChange={(e) => this.handleOtherEvent(e, 3)}
                label="Name of the event"
              />
            </div>
          </div>
          <div className="form_body_grid2 second">
            <h4 ref="hearAbout">
              How did you hear about Sevan Startup Summit 2022? *
            </h4>
            <RadioGroup
              aria-label="aboutSummit"
              name="aboutSummit"
              onChange={this.handleAboutSummit}
            >
              <FormControlLabel
                value="facebook"
                control={<Radio color="primary" />}
              ...
              />
              <FormControlLabel
                value="our industry partners"
                control={<Radio color="primary" />}
                label="Our industry partners"
              />
              <FormControlLabel
                value="previous summit participants"
                control={<Radio color="primary" />}
                label="Previous summit participants"
              />
            </RadioGroup>
          </div>
          <div>
            <div className="textAreaBox long">
              <div>
                <h6 ref="expectations">
                  <i>
                    <img src={require("./img/minIcon/msg.png")} alt="icon" />
                  </i>{" "}
                  What are your expectations from the Summit? *{" "}
                  <span ref="textArea2"> (Max 50 words)</span>
                </h6>
              </div>
              <textarea
                onChange={this.handleExpectationsText}
                maxLength={this.state.expectationsLenght}
              ></textarea>
            </div>
          </div>
        </div>
