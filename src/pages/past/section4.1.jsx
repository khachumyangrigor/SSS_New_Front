...
class PastSpeakers extends React.Component {
  ...
  getInfo = () => {
    ...
    if (location === "/sevan19/speakers" || location === "/sevan19/speakers/")
      return Info_Past6;
    else if (
      location === "/sevan18/speakers" ||
      location === "/sevan18/speakers/"
    )
      return Info_Past1;
    else if (
      location === "/sevan17/speakers" ||
      location === "/sevan17/speakers/"
    )
      return Info_Past2;
    else if (
      location === "/sevan16/speakers" ||
      location === "/sevan16/speakers/"
    )
      return Info_Past3;
    else if (location === "/uae18/speakers" || location === "/uae18/speakers/")
      return Info_Past4;
    else if (
      location === "/india18/speakers" ||
      location === "/india18/speakers/"
    )
      return Info_Past5;
  };
  ...
  render = () => {
    ...
    if (error)
      return (
        <div className="part5Title">
          <div>Error: {error}</div>
        </div>
      );
    return (
      <Fragment>
        <div className="backPage" onClick={() => this.props.history.goBack()}>
        ...
        <div className={`moreInfo ${showInfo}`}>
          <div>
            ...
            <div className="speakerLeftName">{speakerName}</div>
            <div className="speakerLeftPosition">{speakerPosition}</div>
          </div>
          <p>{speakerInfo}</p>
          <div className="vidoClose">
            <span
              onClick={() => this.setState({ showInfo: "", partShowTime: "" })}
            >
              X
            </span>
          </div>
          ...
          >
            {speakers.map((speaker) => (
              <div
                className={`person`}
                key={speaker.id}
                onClick={(e) => this.handelGetinfo(e)}
              >
                <div className="imgMOre">
                  <img
                    src={require(`./img/${API_IMG_URL}/speakers/${speaker.img}`)}
                    alt={speaker.name}
                    rel="img"
                  />
                ...
                <p className="personPosition" hidden={true}>
                  {speaker.position}
                </p>
